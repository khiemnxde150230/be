        ...
        public dynamic GetUserInformation(int userId)
        {
            var userInfo = (from user in _context.Users
                            ...
                            select new
                            {
                                ...
                                bookingTimes = ubg.Count(ub => ub.b.UserId != null),
                                moneySpend = CurrencyFormat(ubg.Where(ub => ub.b.UserId != null).Sum(ub => (decimal?)ub.b.TotalPrice) ?? 0).ToString(),
                                cancelBookings = ubg.Count(ub => ub.b.UserId != null && ub.b.Status == "Canceled")
                            }).FirstOrDefault();
            return userInfo;
        }
