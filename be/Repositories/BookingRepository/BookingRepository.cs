        ...
        public IEnumerable<dynamic> GetBookingByConditions(int userId, string checkin, string checkout, string bookingDate)
        {
            ...
            if (!String.IsNullOrEmpty(bookingDate))
            {
                bookingHistories = (from bookings in bookingHistories
                                    where DateTime.Compare(DateTime.ParseExact(bookings.bookingDate, "yyyy-MM-dd hh:mm tt", null).Date, DateTime.ParseExact(bookingDate, "yyyy-MM-dd", null).Date) == 0
            ...
            {
                bookingHistories = (from bookings in bookingHistories
                                    where DateTime.Compare(DateTime.ParseExact(bookings.checkIn, "yyyy-MM-dd", null), DateTime.ParseExact(checkin, "yyyy-MM-dd", null)) >= 0
                                    && DateTime.Compare(DateTime.ParseExact(bookings.checkOut, "yyyy-MM-dd", null), DateTime.ParseExact(checkout, "yyyy-MM-dd", null)) <= 0
                                    select bookings).Distinct().OrderByDescending(x => x.key).ToList();
            }
            else
            {
                ...
                {
                    bookingHistories = (from bookings in bookingHistories
                                        where bookings.checkIn == DateTime.ParseExact(checkin, "yyyy-MM-dd", null).ToString("yyyy-MM-dd")
                                        select bookings).Distinct().OrderByDescending(x => x.key).ToList();
                }
                ...
                {
                    bookingHistories = (from bookings in bookingHistories
                                        where bookings.checkOut == DateTime.ParseExact(checkout, "yyyy-MM-dd", null).ToString("yyyy-MM-dd")
                                        select bookings).Distinct().OrderByDescending(x => x.key).ToList();
                }
            }
